...
use crate::{
    find_python_path, find_workspace_root, send_sigint,
    PreflightCheckResult, ProcessState, ProjectConfig, SettingsConfig, SourceConfig, TargetConfig,
};

#[tauri::command]
pub fn run_preflight_check(project_dir: String) -> PreflightCheckResult {
...
#[tauri::command]
pub fn open_output_folder(project_dir: String) -> Result<(), String> {
...
#[tauri::command]
pub fn read_project_json(project_dir: String) -> Result<serde_json::Value, String> {
...
#[tauri::command]
pub fn write_project_json(project_dir: String, data: serde_json::Value) -> Result<(), String> {
    let p_file = Path::new(&project_dir).join("project.json");
...
#[tauri::command]
pub fn read_pipeline_log(project_dir: String, limit: usize) -> Result<Vec<String>, String> {
...
#[tauri::command]
pub fn cancel_pipeline(active_proc: State<'_, ProcessState>) -> Result<(), String> {
    let mut lock = active_proc.lock().unwrap();
    if let Some(ref mut child) = lock.child {
        let pid = child.id();
        send_sigint(pid);
...
#[tauri::command]
pub fn list_jobs_queue(status: Option<String>) -> Result<serde_json::Value, String> {
    let ws_root = find_workspace_root().ok_or("Workspace root not found")?;
    let python_path = find_python_path();
...
#[tauri::command]
pub fn pause_job_queue(job_id: String) -> Result<(), String> {
    let ws_root = find_workspace_root().ok_or("Workspace root not found")?;
    let python_path = find_python_path();
...
#[tauri::command]
pub fn read_subtitles(project_dir: String) -> Result<serde_json::Value, String> {
    ...
    if trans_file.exists() {
        ...
        if proj_file.exists() {
            ...
            if let Some(segments) = proj_json.get_mut("segments") {
                if let Some(arr) = segments.as_array_mut() {
                    for (idx, seg) in arr.iter_mut().enumerate() {
                        if seg.get("id").is_none() {
                            if let Some(obj) = seg.as_object_mut() {
                                obj.insert("id".to_string(), serde_json::json!(idx + 1));
                            }
                        }
                    }
                }
...
#[tauri::command]
pub fn write_subtitles(project_dir: String, data: serde_json::Value) -> Result<(), String> {
    let p_dir = PathBuf::from(&project_dir);
...
#[tauri::command]
pub fn run_qc_check(project_dir: String) -> Result<serde_json::Value, String> {
    let ws_root = find_workspace_root().ok_or("Workspace root not found")?;
    let python_path = find_python_path();
...
#[tauri::command]
pub fn apply_autofit_qc(project_dir: String) -> Result<serde_json::Value, String> {
    let ws_root = find_workspace_root().ok_or("Workspace root not found")?;
    let python_path = find_python_path();
...
#[tauri::command]
pub fn preview_tts_voice(text: String, voice: String, gender: String) -> Result<serde_json::Value, String> {
    let ws_root = find_workspace_root().ok_or("Workspace root not found")?;
    let python_path = find_python_path();
...
#[tauri::command]
pub fn read_composition(project_dir: String) -> Result<serde_json::Value, String> {
...
#[tauri::command]
pub fn write_composition(project_dir: String, data: serde_json::Value) -> Result<(), String> {
    let p_dir = PathBuf::from(&project_dir);
